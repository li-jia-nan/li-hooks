...
const createHTMLMediaHook = <T extends HTMLAudioElement | HTMLVideoElement>(tag: Tag) => {
  return (elOrProps: HTMLMediaProps | React.ReactElement<HTMLMediaProps>) => {
    ...
    const wrapEvent = (
      userEvent?: ReactEventHandler<T>,
      proxyEvent?: (e?: React.SyntheticEvent<T, Event>) => void
    ): ReactEventHandler<T> => {
      return event => {
        try {
          proxyEvent?.(event);
    ...
    const onVolumeChange = () => {
      ...
      setState({ muted: el.muted, volume: el.volume });
    };
    const onDurationChange = () => {
      ...
      setState({ duration, buffered: parseTimeRanges(buffered) });
    };
    const onTimeUpdate = () => {
      const el = ref.current;
      if (!el) {
    ...
    useEffect(() => {
      ...
      if (!el) {
        if (process.env.NODE_ENV !== 'production') {
          if (tag === Tag.Audio) {
            console.error(
              '[createHTMLMediaHook]: useAudio() ref to <audio> element is empty at mount. ' +
          ...
          if (tag === Tag.Video) {
            console.error(
              '[createHTMLMediaHook]: useVideo() ref to <video> element is empty at mount. ' +
